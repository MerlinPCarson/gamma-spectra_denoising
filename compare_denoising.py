...
def compare_denoising(spectra, denoised_spectra, minkev, maxkev, show_figs=True, save_figs=False):
    for spec in glob(os.path.join(denoised_spectra, '*.json')):
        ...
        ax[0][0].plot(keV, hits, color='blue', label='$^{239}$Pu spectrum', alpha=0.6)
        ax[0][0].plot(keV, hitsBS, color='green', label='background subtracted', alpha=0.6)
        ax[0][0].legend(fancybox=True, shadow=True, fontsize=16)
        ax[0][0].set_xlabel('energy (keV)', fontname='cmtt10', fontsize=22, fontweight='bold')
        ax[0][0].set_ylabel('Intensity', fontname='cmtt10', fontsize=22, fontweight='bold')
        ax[0][0].set_xticks(np.arange(minkev, maxkev, 100))
        ax[0][0].set_xticks(np.arange(minkev, maxkev, 20), minor=True)
        ax[0][0].grid(axis='x', which='major', alpha=0.5)
        ax[0][0].grid(axis='x', which='minor', alpha=0.2)
        ax[0][0].set_xlim(minkev,maxkev)
        ax[0][1].plot(keV, hits, color='blue', label='$^{239}$Pu spectrum', alpha=0.6)
        ax[0][1].plot(keVDN, hitsDN, color='red', label='GS-DnCNN denoised', alpha=0.6)
        ax[0][1].set_xlabel('energy (keV)', fontname='cmtt10', fontsize=22, fontweight='bold')
        ax[0][1].set_ylabel('Intensity', fontname='cmtt10', fontsize=22, fontweight='bold')
        ax[0][1].set_xticks(np.arange(minkev, maxkev, 100))
        ax[0][1].set_xticks(np.arange(minkev, maxkev, 20), minor=True)
        ax[0][1].grid(axis='x', which='major', alpha=0.5)
        ax[0][1].grid(axis='x', which='minor', alpha=0.2)
        ax[0][1].set_xlim(minkev,maxkev)
        ax[0][1].legend(fancybox=True, shadow=True, fontsize=16)

        # compare background subtracted with DNN denoised spectrum
        ax[1][0].plot(keV, hitsBS, color='green', label='background subtracted', alpha=0.6)
        ax[1][0].plot(keV, hitsDN, color='red', label='GS-DnCNN denoised', alpha=0.6)
        ax[1][0].legend(fancybox=True, shadow=True, fontsize=16)
        ax[1][0].set_xlabel('energy (keV)', fontname='cmtt10', fontsize=22, fontweight='bold')
        ax[1][0].set_ylabel('Intensity', fontname='cmtt10', fontsize=22, fontweight='bold')
        ax[1][0].set_xticks(np.arange(minkev, maxkev, 100))
        ax[1][0].set_xticks(np.arange(minkev, maxkev, 20), minor=True)
        ax[1][0].grid(axis='x', which='major', alpha=0.5)
        ax[1][0].grid(axis='x', which='minor', alpha=0.2)
        ax[1][0].set_xlim(minkev,maxkev)
        ax[1][1].plot(keV, hitsBSNorm, color='green', label='normalized background subtracted', alpha=0.6)
        ax[1][1].plot(keVDN, hitsDNNorm, color='red', label='normalized GS-DnCNN denoised', alpha=0.6)
        ax[1][1].set_xlabel('energy (keV)', fontname='cmtt10', fontsize=22, fontweight='bold')
        ax[1][1].set_ylabel('Intensity', fontname='cmtt10', fontsize=22, fontweight='bold')
        ax[1][1].set_xticks(np.arange(minkev, maxkev, 100))
        ax[1][1].set_xticks(np.arange(minkev, maxkev, 20), minor=True)
        ax[1][1].grid(axis='x', which='major', alpha=0.5)
        ax[1][1].grid(axis='x', which='minor', alpha=0.2)
        ax[1][1].set_xlim(minkev,maxkev)
        ax[1][1].legend(fancybox=True, shadow=True, fontsize=16)
        ...
        if save_figs:
            out_file = os.path.join(denoised_spectra, os.path.basename(spec).replace('.json', '.pdf'))
