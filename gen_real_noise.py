...
from spectra_utils import load_nndc_tables, load_radionuclide_nndc
...
def generate_spectra(config, nndc_tables, params, outdir):
    ...
    for rn_name, rn_values in tqdm(nndc_tables.items()):
        for compton_scale, snr in params:
            spectrum_keV, spectrum, noisy_spectrum, noise = generate_spectrum_SNR(rn_values, config, compton_scale=compton_scale, snr=snr)
...
def main():
    ...
    parser.add_argument("-nc", "--numcompton", help="number of compton scales between min and max compton scale", default=6, type=int)
